        ...
        <div className="hidden md:flex items-center gap-8">
          {navLinks.map((link) => (
            <a 
              key={link.name} 
              href={link.href} 
              className="text-gray-400 hover:text-brand-gold transition-colors font-medium text-sm"
            >
              {link.name}
            </a>
          ))}
          <button className="bg-gradient-to-r from-brand-red to-brand-gold px-6 py-2.5 rounded-full text-white font-bold text-sm hover:shadow-[0_0_20px_rgba(255,75,75,0.5)] transition-all transform hover:scale-105">
            Get Started
          </button>
        </div>
        ...
        {mobileMenuOpen && (
          ...
          >
            <div className="flex flex-col p-6 gap-4">
              ...
              <button className="w-full bg-brand-red py-3 rounded-lg font-bold">Get Started</button>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
  ...
  return (
    <div className="relative w-full max-w-2xl aspect-video bg-gray-900 rounded-2xl border border-white/10 shadow-2xl overflow-hidden glass-card">
      ...
      <div className="absolute inset-0 top-12 left-16 bg-[#0f172a] p-4 flex items-center justify-center">
        {/* Fake Map Grid */}
        <div className="absolute inset-0 opacity-20" style={{ backgroundImage: 'radial-gradient(circle, #ff4b4b 1px, transparent 1px)', backgroundSize: '30px 30px' }}></div>
        ...
        <div className="absolute bottom-4 right-4 w-40 h-24 bg-navy-black/80 backdrop-blur-lg border border-white/10 rounded-xl p-3 flex flex-col gap-2">
          ...
          <div className="mt-auto flex justify-between">
            <div className="w-8 h-8 rounded bg-brand-red/20"></div>
            <div className="w-8 h-8 rounded bg-brand-gold/20"></div>
          </div>
        </div>
      </div>
    </div>
...
const StatItem = ({ label, value, subtext }) => {
  return (
    ...
    >
      <h3 className="text-sm font-bold text-brand-gold uppercase tracking-widest mb-1">{label}</h3>
...
const FeatureCard = ({ icon: Icon, title, description, index }) => {
  return (
    ...
    >
      ...
      <div className="bg-brand-red/10 w-14 h-14 rounded-2xl flex items-center justify-center mb-6 group-hover:bg-brand-red/20 transition-colors">
        <Icon className="text-brand-red w-7 h-7" />
      </div>
...
const Step = ({ number, title, description, isLast }) => {
  return (
    <div className="flex-1 relative flex flex-col items-center text-center">
      <div className="w-16 h-16 rounded-full bg-gradient-to-br from-brand-red to-brand-gold flex items-center justify-center text-2xl font-black mb-6 relative z-10 shadow-[0_0_20px_rgba(255,75,75,0.3)]">
        {number}
      </div>
      {!isLast && (
        <div className="hidden lg:block absolute top-8 left-[calc(50%+2rem)] w-[calc(100%-4rem)] h-[2px] bg-gradient-to-r from-brand-red/50 to-transparent">
          <motion.div 
            initial={{ scaleX: 0 }}
...
const TechBadge = ({ name, emoji }) => {
  return (
    <div className="px-5 py-2.5 rounded-full bg-white/5 border border-white/10 flex items-center gap-2 hover:border-brand-red/50 hover:bg-white/10 transition-all cursor-default group">
...
const App = () => {
  return (
    <div className="bg-navy-black text-white min-h-screen selection:bg-brand-red/30 selection:text-white overflow-x-hidden">
      <Navbar />

      {/* --- HERO SECTION --- */}
      <section className="relative pt-32 pb-20 px-6 lg:pt-48 lg:pb-32 overflow-hidden">
        {/* Background Gradients */}
        <div className="absolute top-0 left-1/2 -translate-x-1/2 w-full h-full bg-[radial-gradient(circle_at_top,_var(--tw-gradient-stops))] from-brand-red/10 via-transparent to-transparent -z-10"></div>
        ...
        <div className="max-w-7xl mx-auto grid lg:grid-cols-2 gap-16 items-center">
          ...
          >
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              <button className="bg-gradient-to-r from-brand-red to-brand-gold px-8 py-4 rounded-xl text-white font-black text-lg flex items-center justify-center gap-2 hover:shadow-[0_0_30px_rgba(255,75,75,0.4)] transition-all transform hover:scale-105 active:scale-95 group">
      ...
      <section id="features" className="py-24 px-6 relative">
        <div className="max-w-7xl mx-auto">
          <div className="text-center max-w-3xl mx-auto mb-20">
            <h2 className="text-sm font-black text-brand-red uppercase tracking-widest mb-4">Core Capabilities</h2>
      ...
      <section id="contact" className="py-32 px-6 relative overflow-hidden">
        ...
        <div className="max-w-4xl mx-auto text-center glass-card p-12 lg:p-20 rounded-[40px]">
          ...
          <button className="bg-brand-red text-white px-12 py-5 rounded-2xl font-black text-xl hover:shadow-[0_0_40px_rgba(255,75,75,0.6)] transition-all transform hover:scale-105 active:scale-95 flex items-center justify-center gap-3 mx-auto group">
